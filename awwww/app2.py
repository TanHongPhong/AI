...
def extract_video_frames(video_path: str, num_frames: int = 4) -> dict:
    """Extract frames from video that contain readable QR codes
    Returns dict with frames and scan logs for UI display
    """
    import base64
    ...
    logs = []
    logs.append(f"📹 Video loaded: {total_frames} total frames")
    
    if total_frames <= 0:
        cap.release()
        logs.append("❌ Failed to read video")
        return {"frames": [], "logs": logs}
    ...
    logs.append(f"🔍 Scanning {len(sample_indices)} frames for QR codes...")
    ...
    for idx in sample_indices:
        ...
        if ret:
            ...
            if qr_text:  # QR found
                logs.append(f"✅ Frame {idx}: QR detected → '{qr_text}'")
    ...
    logs.append(f"📊 Scanned {frames_scanned} frames, found {len(frames_with_qr)} with QR")
    
    # If no QR frames found, fallback to evenly spaced frames
    if not frames_with_qr:
        logs.append("⚠️ No QR found! Using evenly spaced frames as fallback")
        cap = cv2.VideoCapture(video_path)
    ...
    return {"frames": frames_with_qr, "logs": logs}
...
async def extract_frames(
    ...
    try:
        ...
        result = extract_video_frames(video_path, num_frames)
        ...
        return {"success": False, "error": str(e), "logs": [f"❌ Error: {str(e)}"]}
