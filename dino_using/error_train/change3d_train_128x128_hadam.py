...
- NEW: optional Hadamard (⊙) channel in addition to Δ (enable with --add_hadamard)
"""
...
class PairedAugment:
    ...
    def __call__(self, img1: Image.Image, img2: Image.Image, label: int, add_delta: bool = False, add_hadamard: bool = False) -> torch.Tensor:
...
class DirPairsDataset(Dataset):
    def __init__(self, split_dir: str, classes: List[str], class_to_id: Dict[str, int],
                 ...
                 diff_photometric_ids: List[int], add_hadamard: bool = False):
    ...
    def __getitem__(self, idx):
        ...
        if self.augmenter is not None:
            x = self.augmenter(img1, img2, y, add_delta=self.add_delta, add_hadamard=self.add_hadamard)
        else:
            size = 128
            ...
            if self.add_hadamard:
                hadam = t2 * t1
                x = torch.cat([x, hadam.unsqueeze(1).repeat(1,2,1,1)], dim=0)
        return x, y
...
def save_val_samples(val_loader: DataLoader, out_dir: str, max_samples: int = 8):
    ...
    for x, y in val_loader:
        ...
        for i in range(N):
            ...
            tensor_to_pil(hadam).save(base + "_hadam.png")
        break
...
def parse_args() -> Args:
    ...
    # Δ channel
    p.add_argument('--add_delta', action='store_true', help='Add delta channel (enabled by default in this script)')
    p.add_argument('--no-add-delta', dest='add_delta', action='store_false', help='Disable delta channel')
    # ⊙ channel
    p.add_argument('--add_hadamard', action='store_true', help='Add Hadamard (⊙) channel')
    p.add_argument('--no-add-hadamard', dest='add_hadamard', action='store_false', help='Disable Hadamard channel')
    p.set_defaults(add_delta=True, add_hadamard=True)
...
def build_loaders(args: Args):
    ...
    train_ds = DirPairsDataset(train_dir, classes, class_to_id, augmenter=aug_train, add_delta=args.add_delta, diff_photometric_ids=diff_ids, add_hadamard=args.add_hadamard)
    val_ds   = DirPairsDataset(val_dir,   classes, class_to_id, augmenter=aug_eval,  add_delta=args.add_delta, diff_photometric_ids=diff_ids, add_hadamard=args.add_hadamard)
    ...
    if os.path.isdir(test_dir):
        try:
            test_ds = DirPairsDataset(test_dir, classes, class_to_id, augmenter=aug_eval, add_delta=args.add_delta, diff_photometric_ids=diff_ids, add_hadamard=args.add_hadamard)
...
def main():
    ...
    in_ch = 3 + (3 if args.add_delta else 0) + (3 if args.add_hadamard else 0)
    ...
    print(f"[INFO] Channels: base RGB=3, +Δ={'ON' if args.add_delta else 'OFF'}, +⊙={'ON' if args.add_hadamard else 'OFF'} -> in_ch={in_ch}")
