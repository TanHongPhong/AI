...
def compare_images(img1_path, img2_path, ckpt_path=None, sim_threshold=None, save_prefix="diff"):
    ...
    pil1 = resize_to_multiple(pil_load(img1_path))
    pil2 = resize_to_multiple(pil_load(img2_path))
    t1, t2 = to_tensor(pil1), to_tensor(pil2)
    H, W = t1.shape[-2:]
    f1, (Hp, Wp) = extract_patch_tokens(model, t1)
    f2, _        = extract_patch_tokens(model, t2)
    # 4.1) Trực quan hóa feature map bằng trung bình kênh
    def feat_to_vis(feat):  # feat: [Hp, Wp, D]
        feat_avg = feat.mean(axis=-1)  # [Hp, Wp]
        feat_up = upsample_to_img(feat_avg.numpy(), (H, W))  # [H,W]
        norm = (feat_up - feat_up.min()) / (feat_up.max() - feat_up.min() + 1e-8)
        vis = (norm * 255).astype(np.uint8)
        return cv2.applyColorMap(vis, cv2.COLORMAP_JET)
    feat_vis1 = feat_to_vis(f1)
    feat_vis2 = feat_to_vis(f2)
    ...
    if sim_threshold is not None:
        ...
        mask = (sim_up < float(sim_threshold)).astype(np.uint8) * 255
    else:
        mask = otsu_mask(diff_up)

    # Vẽ bbox lên cả ảnh gốc 1, gốc 2, và overlay
    ...
    out_heat     = os.path.join("outputs/sum", f"{save_prefix}_heat.jpg")
    out_ovly     = os.path.join("outputs/sum", f"{save_prefix}_overlay.jpg")
    out_mask     = os.path.join("outputs/sum", f"{save_prefix}_mask.png")
    out_boxed1   = os.path.join("outputs/sum", f"{save_prefix}_boxed_img1.jpg")
    out_boxed2   = os.path.join("outputs/sum", f"{save_prefix}_boxed_img2.jpg")
    out_feat1 = os.path.join("outputs/sum", f"{save_prefix}_feat1.jpg")
    out_feat2 = os.path.join("outputs/sum", f"{save_prefix}_feat2.jpg")
    cv2.imwrite(out_feat1, feat_vis1)
    cv2.imwrite(out_feat2, feat_vis2)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--img1", default="outputs/sum/04_template_roi.png")
    ap.add_argument("--img2", default="outputs/sum/05_moving_aligned_roi.png")
    ap.add_argument("--ckpt", default=".cache/checkpoints/dinov2_vitb14_reg4_pretrain.pth")
    ap.add_argument("--sim_threshold", type=float, default=0.45)
