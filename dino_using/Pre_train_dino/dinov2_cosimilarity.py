...
def crop_image_around_center(img, center_x, center_y, crop_size=96):
    ...
    # Nếu ảnh crop nhỏ hơn crop_size, pad với màu đen
...
def load_json_bboxes(json_path):
    """
    Đọc file JSON chứa thông tin bounding boxes
    """
...
def compare_images(img1_path, img2_path, ckpt_path=None, sim_threshold=None, save_prefix="diff"):
    """
    So sánh hai ảnh và tìm các vùng khác biệt
    """
    ...
    # Tính toán threshold và tạo mask
    ...
    # Lưu thông tin bounding boxes vào JSON
    bbox_data = {
        "image_info": {
            "img1_path": img1_path,
            "img2_path": img2_path,
...
def crop_bbox_images(json_path, output_dir, crop_size=96, img_source="img1"):
    """
    Đọc JSON và cắt các ảnh 96x96px xung quanh tâm của bounding boxes
    
    Args:
        json_path: Đường dẫn đến file JSON
        output_dir: Thư mục lưu ảnh crop
        crop_size: Kích thước ảnh crop (mặc định 96x96)
        img_source: "img1" hoặc "img2" - ảnh nào để crop
    """
    ...
    for bbox in bboxes:
        ...
        # Crop ảnh xung quanh tâm
        cropped = crop_image_around_center(img, center_x, center_y, crop_size)
        
        # Tạo tên file
        filename = f"bbox_{bbox_id:03d}_center_{center_x}_{center_y}_{crop_size}x{crop_size}.jpg"
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        ...
        for i, bbox in enumerate(bboxes):
            f.write(f"Bbox {bbox['id']}:\n")
            f.write(f"  Position: ({bbox['position']['x']}, {bbox['position']['y']})\n")
            f.write(f"  Size: {bbox['size']['width']}x{bbox['size']['height']}\n")
            f.write(f"  Center: ({bbox['center']['x']}, {bbox['center']['y']})\n")
            f.write(f"  Area: {bbox['area']}\n")
            f.write(f"  Output: {os.path.basename(cropped_images[i])}\n\n")
...
def full_workflow(img1_path, img2_path, ckpt_path=None, sim_threshold=0.35, 
    ...
    """
    Workflow hoàn chỉnh: so sánh ảnh -> tạo JSON -> crop ảnh
    
    Args:
        img1_path: Đường dẫn ảnh template
        img2_path: Đường dẫn ảnh moving
        ckpt_path: Đường dẫn checkpoint DINOv2 (optional)
        sim_threshold: Ngưỡng similarity
        save_prefix: Prefix cho file output
        crop_size: Kích thước crop (mặc định 96)
        crop_source: "img1" hoặc "img2" để crop
        crop_output_dir: Thư mục lưu ảnh crop (mặc định tự động)
    """
    print("🚀 Bắt đầu workflow hoàn chỉnh...")
    
    # Bước 1: So sánh ảnh và tạo JSON
...
# ================== MAIN ==================
def main():
    parser = argparse.ArgumentParser(description="DINOv2 Cosine Similarity Analysis & Image Cropping")
    parser.add_argument("--mode", choices=["compare", "crop", "full"], default="full",
                       help="Chế độ chạy: compare (chỉ so sánh), crop (chỉ crop), full (đầy đủ)")
    
    # Arguments cho compare mode
    parser.add_argument("--img1", default=r"data_canonical_1\le1\013071__f000230__deskew171deg__rot180__1230x735.png", 
                       help="Đường dẫn ảnh template")
    parser.add_argument("--img2", default=r"data_canonical_1\le1\013071__f000000__deskew25deg__rot0__1206x717.png", 
                       help="Đường dẫn ảnh moving")
    parser.add_argument("--ckpt", default=r".cache\checkpoints\dinov2_vitb14_reg4_pretrain.pth",
                       help="Đường dẫn checkpoint DINOv2")
    parser.add_argument("--sim_threshold", type=float, default=0.35,
                       help="Ngưỡng similarity")
    parser.add_argument("--prefix", default="diff", help="Prefix cho file output")
    
    # Arguments cho crop mode
    parser.add_argument("--json", default=r"outputs\sum\cos_sim\diff_bboxes.json",
                       help="Đường dẫn file JSON (cho crop mode)")
    ...
    if args.mode == "compare":
        ...
        # Chỉ crop ảnh từ JSON
        if not os.path.exists(args.json):
            print(f"[ERROR] File JSON không tồn tại: {args.json}")
            print(f"[INFO] Hãy chạy với --mode compare trước để tạo file JSON")
            return
        crop_bbox_images(args.json, args.crop_output, args.crop_size, args.crop_source)
        
    elif args.mode == "full":
        # Workflow đầy đủ
        full_workflow(args.img1, args.img2, args.ckpt, args.sim_threshold, 
                     args.prefix, args.crop_size, args.crop_source, args.crop_output)
