...
def crop_bbox_images(json_path, output_dir, crop_size=96, img_source="img1", prefix_for_files=None):
    ...
    for bbox in bbox_data["bounding_boxes"]:
        ...
        base_name = f"bbox_{bbox_id:03d}_center_{cx}_{cy}_{crop_size}x{crop_size}.jpg"
        filename = f"{prefix_for_files}__{base_name}" if prefix_for_files else base_name
...
def pair_same_names(dir_a, dir_b):
    ...
    return [(a[k], b[k], k) for k in common]

def main():
    parser = argparse.ArgumentParser(description="DINOv2 Batch - Same Filenames")
    parser.add_argument("--dir1", required=True, help="Thư mục A")
    parser.add_argument("--dir2", required=True, help="Thư mục B")
    ...
    pairs = pair_same_names(args.dir1, args.dir2)
    if args.limit_pairs:
