...
def compare_images(img1_path, img2_path, ckpt_path=None, sim_threshold=None, save_prefix="diff", model=None):
    # Load model once if not provided (fallback for standalone usage)
    if model is None:
...
def crop_bbox_images_both(json_path, output_dir_before, output_dir_after, crop_size=96, prefix_for_files=None):
    ...
    for bbox in bbox_data["bounding_boxes"]:
        ...
        cx = bbox["center"]["x"]
        cy = bbox["center"]["y"]
        # Short, consistent filename across before/after, no coords/size
...
def list_image_subdirs(root_dir):
    ...
    if not os.path.isdir(root_dir):
        return subdirs
    # Include any subdirectories that have images
    for name in sorted(os.listdir(root_dir)):
        sub = os.path.join(root_dir, name)
        if os.path.isdir(sub) and len(list_images(sub)) > 0:
            subdirs.append(sub)
    # Prefer only le1-le4 and tao1-tao4 if available; otherwise keep all with images
...
def resolve_image_dir(root_dir):
    # If the path itself contains images, use it directly
    if os.path.isdir(root_dir):
        imgs = list_images(root_dir)
        if len(imgs) > 0:
            return root_dir
        # Prefer `le1/` if present
        le1 = os.path.join(root_dir, "le1")
        if os.path.isdir(le1) and len(list_images(le1)) > 0:
            return le1
        # Otherwise pick the first subdir that has images
        for name in sorted(os.listdir(root_dir)):
            sub = os.path.join(root_dir, name)
            if os.path.isdir(sub) and len(list_images(sub)) > 0:
                return sub
...
def main():
    parser = argparse.ArgumentParser(description="DINOv2 Batch - Self Cross (within one folder) - ViT-S")
    parser.add_argument("--dir", default=r"data_canonical_3", help="Thư mục chứa ảnh")
    ...
    # Determine all image directories to process (root and its subfolders with images)
    img_dirs = list_image_subdirs(args.dir)
    if not img_dirs:
        # Fallback to single resolution if nothing found by enumeration
        single_dir = resolve_image_dir(args.dir)
        img_dirs = [single_dir]
    ...
    for img_dir in img_dirs:
        sub_name = os.path.basename(os.path.normpath(img_dir))
        print(f"\n==== [DIR] Processing: {img_dir} ====")
        pairs = pair_self(img_dir, ordered=args.ordered)
        if args.limit_pairs:
            pairs = pairs[:args.limit_pairs]
        total = len(pairs)
        grand_total += total
        for idx, (pa, pb, name) in enumerate(pairs, 1):
            print(f"\n==== [BATCH SELF VITS] {sub_name} {idx}/{total}: {name} ====")
            save_prefix = f"{args.prefix}_{sub_name}_{name}"
            json_path, boxes = compare_images(pa, pb, None, args.sim_threshold, save_prefix, model=model)
            if boxes:
                crop_bbox_images_both(json_path, args.outputs_crop_before, args.outputs_crop_after, args.crop_size, prefix_for_files=f"{sub_name}__{name}")
            else:
                print("[INFO] No boxes, skip cropping.")
    print(f"\n[FINISHED] {grand_total} pairs processed across {len(img_dirs)} folder(s).")
