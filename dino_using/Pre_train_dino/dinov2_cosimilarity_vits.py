...
def draw_bboxes(img_bgr, mask_u8, min_area_ratio=MIN_BBOX_AREA, min_wh=PATCH_SIZE):
    ...
    for c in cnts:
        ...
        # 1) lọc theo tỉ lệ diện tích
        if w * h < min_area:
            continue
        # 2) lọc hộp nhỏ hơn 2 patch
...
def compare_images(img1_path, img2_path, ckpt_path=None, sim_threshold=None, save_prefix="diff"):
    """
    So sánh hai ảnh và tìm các vùng khác biệt (ViT-S)
    """
    ...
    cv2.imwrite(out_heat, colorize_heatmap(diff_up))
...
def full_workflow(img1_path, img2_path, ckpt_path=None, sim_threshold=0.35, 
    ...
    """
    Workflow hoàn chỉnh: so sánh ảnh -> tạo JSON -> crop ảnh (ViT-S)
    """
...
# ================== MAIN ==================
def main():
    parser = argparse.ArgumentParser(description="DINOv2 Cosine Similarity (ViT-S) Analysis & Image Cropping")
    ...
    parser.add_argument("--img2", default=r"data_canonical_1\le1\013071__f000460__deskew5deg__rot180__1240x733.png",
                       help="Đường dẫn ảnh moving")
    parser.add_argument("--ckpt", default=r".cache\checkpoints\dinov2_vits14_reg4_pretrain.pth",
                       help="Đường dẫn checkpoint DINOv2 ViT-S")
