...
def draw_bboxes(img_bgr, mask_u8, min_area_ratio=MIN_BBOX_AREA, min_wh=PATCH_SIZE):
    ...
    for c in cnts:
        ...
        if w * h < ((PATCH_SIZE * 2) * (PATCH_SIZE * 2)) or w < min_wh * 2 or h < min_wh * 2:
            continue
        # Giới hạn bbox tối đa 250x250, giữ tâm và clamp trong ảnh
        max_w, max_h = 250, 250
        cx = x + w // 2
        cy = y + h // 2
        new_w = min(w, max_w)
        new_h = min(h, max_h)
        nx = max(0, min(W - new_w, cx - new_w // 2))
        ny = max(0, min(H - new_h, cy - new_h // 2))
        nx = int(nx)
        ny = int(ny)
        new_w = int(new_w)
        new_h = int(new_h)
        boxes.append((nx, ny, new_w, new_h))
        cv2.rectangle(out, (nx, ny), (nx + new_w, ny + new_h), (0, 255, 0), 2)
    return out, boxes
...
def compare_images(img1_path, img2_path, ckpt_path=None, sim_threshold=None, save_prefix="diff", model=None):
    ...
    thr_out = thr_in / 2.0
    mask, (x0, y0, rw, rh) = make_roi_mask(sim_up, roi_w=1000, roi_h=440, thr_in=thr_in, thr_out=thr_out)
    cv2.rectangle(overlay_bgr, (x0, y0), (x0+rw, y0+rh), (255, 255, 255), 2)
    out_dir = os.path.join("outputs", "cross_sum", "cos_sim")
...
def crop_bbox_images_both(json_path, output_dir_before, output_dir_after, crop_size=96, prefix_for_files=None):
    ...
    for bbox in bbox_data["bounding_boxes"]:
        bbox_id = bbox["id"]
        # Tính tâm từ (x, y, w, h) để đảm bảo cắt đúng tâm bbox
        x = int(bbox["position"]["x"])
        y = int(bbox["position"]["y"])
        w = int(bbox["size"]["width"])
        h = int(bbox["size"]["height"])
        cx = x + w // 2
        cy = y + h // 2
...
def crop_bbox_images(json_path, output_dir, crop_size=128, img_source="img1", prefix_for_files=None):
    bbox_data = load_json_bboxes(json_path)
...
def main():
    ...
    parser.add_argument("--dir1", default=r"data_canonical_3", help="data_canonical_3")
    parser.add_argument("--dir2", default=r"data_canonical_4", help="data_canonical_4")
    ...
    parser.add_argument("--outputs_cos", default=r"outputs\cross_sum\cos_sim")
    parser.add_argument("--outputs_crop", default=r"outputs\cross_sum\cropped_images")
    parser.add_argument("--outputs_crop_before", default=r"outputs\cross_sum\cropped_images\before")
    parser.add_argument("--outputs_crop_after", default=r"outputs\cross_sum\cropped_images\after")
    parser.add_argument("--limit_pairs", type=int, default=0, help="Giới hạn số cặp (0 = không giới hạn)")
    ...
    # Discover subfolders and cross only matching names
    dirs1 = list_image_subdirs(args.dir1) or [args.dir1]
    dirs2 = list_image_subdirs(args.dir2) or [args.dir2]
    ...
    for key in common_keys:
        dir_a = map1[key]
        dir_b = map2[key]
        print(f"\n==== [GROUP] Matching '{key}': {dir_a}  x  {dir_b} ====")
        ...
        for idx, (pa, pb, name) in enumerate(pairs, 1):
            print(f"\n==== [BATCH CROSS VITS] {key} {idx}/{total}: {name} ====")
            save_prefix = f"{args.prefix}_{key}_{name}"
            json_path, boxes = compare_images(pa, pb, None, args.sim_threshold, save_prefix, model=model)
            if boxes:
                crop_bbox_images_both(json_path, args.outputs_crop_before, args.outputs_crop_after, args.crop_size, prefix_for_files=f"{key}__{name}")
            else:
                print("[INFO] No boxes, skip cropping.")
    print(f"\n[FINISHED] {grand_total} pairs processed across {len(common_keys)} matched folder(s).")
